...
describe('HeadquartersRepository', () => {
  ...
  describe('create', () => {
    it('should create a new headquarters and return it', async () => {
      ...
      expect(mockDb.run).toHaveBeenCalledWith(
        'INSERT INTO headquarters (name, description, address, contact_person, email, phone) VALUES (?, ?, ?, ?, ?, ?)',
        ['New HQ', 'New headquarters', '456 New St', 'Jane Smith', 'jane@newhq.com', '555-5678'],
      );
  ...
  describe('update', () => {
    it('should update existing headquarters and return updated data', async () => {
      ...
      expect(mockDb.run).toHaveBeenCalledWith(
        'UPDATE headquarters SET name = ? WHERE headquarters_id = ?',
        ['Updated HQ', 1],
      );
